...
if (theme_get_setting('frontman_rebuild_registry')) {
  drupal_rebuild_theme_registry();
}

// Add Zen Tabs styles
if (theme_get_setting('frontman_zen_tabs')) {
  drupal_add_css( drupal_get_path('theme', 'frontman') .'/css/tabs.css', 'theme', 'screen');
}
...
function frontman_preprocess_page(&$vars, $hook) {
  ...
  if (user_access('administer blocks')) {
	...
	if (theme_get_setting('frontman_wireframe')) {
  ...
  if (!$vars['is_front']) {
    ...
    $body_classes[] = frontman_id_safe('page-'. $path);
    $body_classes[] = frontman_id_safe('section-'. $section);
...
function frontman_preprocess_node(&$vars, $hook) {
  ...
  $classes[] = frontman_id_safe('node-type-' . $vars['type']);
...
function frontman_preprocess_block(&$vars, $hook) {
    $block = $vars['block'];

    // special block classes
    $classes = array('block');
    $classes[] = frontman_id_safe('block-' . $vars['block']->module);
    $classes[] = frontman_id_safe('block-' . $vars['block']->region);
    $classes[] = frontman_id_safe('block-id-' . $vars['block']->bid);
    $classes[] = 'clearfix';
    ...
    if (theme_get_setting('frontman_block_editing') && user_access('administer blocks')) {
...
function frontman_preprocess_comment(&$vars, $hook) {
...
function frontman_menu_item_link($link) {
...
function frontman_menu_local_tasks() {
  $output = '';
...
function frontman_menu_item($link, $has_children, $menu = '', $in_active_trail = FALSE, $extra_class = NULL) {
...
#New line added to get unique classes for each menu item
  $css_class = frontman_id_safe(str_replace(' ', '_', strip_tags($link)));
...
function frontman_id_safe($string) {
...
function frontman_breadcrumb($breadcrumb) {
  if (!empty($breadcrumb)) {
