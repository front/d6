...
// Add Zen Tabs styles
if (theme_get_setting('STARTER_zen_tabs')) {
  drupal_add_css( drupal_get_path('theme', 'STARTER') .'/css/tabs.css', 'theme', 'screen');
}
...
function STARTER_preprocess_page(&$vars, $hook) {
  ...
  if (user_access('administer blocks')) {
	...
	if (theme_get_setting('STARTER_wireframe')) {
  ...
  if (!$vars['is_front']) {
    ...
    $body_classes[] = STARTER_id_safe('page-'. $path);
    $body_classes[] = STARTER_id_safe('section-'. $section);
...
function STARTER_preprocess_node(&$vars, $hook) {
  ...
  $classes[] = STARTER_id_safe('node-type-' . $vars['type']);
...
function STARTER_preprocess_block(&$vars, $hook) {
    ...
    $classes = array('block');
    $classes[] = STARTER_id_safe('block-' . $vars['block']->module);
    $classes[] = STARTER_id_safe('block-' . $vars['block']->region);
    $classes[] = STARTER_id_safe('block-id-' . $vars['block']->bid);
    $classes[] = 'clearfix';
    ...
    if (theme_get_setting('STARTER_block_editing') && user_access('administer blocks')) {
...
function STARTER_menu_local_tasks() {
  $output = '';
...
function STARTER_menu_item($link, $has_children, $menu = '', $in_active_trail = FALSE, $extra_class = NULL) {
...
#New line added to get unique classes for each menu item
  $css_class = STARTER_id_safe(str_replace(' ', '_', strip_tags($link)));
...
function STARTER_id_safe($string) {
...
function STARTER_breadcrumb($breadcrumb) {
  if (!empty($breadcrumb)) {
